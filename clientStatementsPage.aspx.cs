...
{
    public partial class clientStatementsPage : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (dropListOrderBy.SelectedValue != null && (CheckBoxNo.Checked || CheckBoxYes.Checked))
            {
                ...
                string query = "SELECT FirstName,Surname,ContactNumber,EmailAddress,DateOfBirth,Username FROM Clients WHERE DateJoined >= @startingPeriod AND DateJoined <= @endingPeriod ORDER BY " + orderBy;
                ...
                {
                    string filePath = Path.Combine(Path.GetTempPath(), "ClientStatement.txt");

                    try
                    {
                        using (StreamWriter writer = new StreamWriter(filePath))
                        {
                            int[] widths = { 20, 20, 17, 30, 15, 20 };
                            writer.WriteLine("Professional Banking Services: Client Statement Per Time Period");
                            writer.WriteLine();
                            ...
                            writer.WriteLine("First Name".PadRight(widths[0]) +
                                            ...
                                            "Contact Number".PadRight(widths[2]) +
                                            "Email Address".PadRight(widths[3]) +
                                            "Date of Birth".PadRight(widths[4]) +
                                            "Username".PadRight(widths[5]));
                            writer.WriteLine();
                            ...
                            foreach (GridViewRow row in gridViewData.Rows)
                            {
                                if (row.RowType == DataControlRowType.DataRow)
                                {
                                    writer.Write(
                                        ...
                                        row.Cells[5].Text.PadRight(widths[5])
                                    );
                                    writer.WriteLine();
                                }
                            }
                        ...
                        emailClientStatement();
                    }
                    catch (IOException ex)
                    {
                        lblReportTitle.Text = "An error occurred generating the client statement: " + ex.Message;
                        lblReportTitle.Visible = true;
                    }
                }
        ...
        public void emailClientStatement()
        {
            string password = "bcyr bxfe rjyw uraf";
            string email = "jdoe29803@gmail.com";
            string filePath = Path.Combine(Path.GetTempPath(), "ClientStatement.txt");
            ...
            try
            {
                ...
                MailMessage mailMessage = new MailMessage
                {
                    ...
                    Subject = "Professional Banking Services - Client Statement Per Time Period Joined",
                    Body = "Please find attached your client statement as requested: ",
                    IsBodyHtml = false,
                };

                mailMessage.To.Add("46677828@mynwu.ac.za");
                mailMessage.Attachments.Add(new Attachment(filePath));
                smtpClient.Send(mailMessage);
            }
            catch (SqlException ex)
            {
                lblReportTitle.Text = "An error occcured emailing the client statement: " + ex.Message;
                lblReportTitle.Visible = true;
            }
        }
